class TestData() :

	CHROME_EXECUTABLE_PATH = "/usr/local/bin/chromedriver"
	BASE_URL = "https://demo.applitools.com/hackathonV2.html"
	FLASH_SALE_URL = "https://demo.applitools.com/hackathonAppV2.html?showAd=true"
	API_KEY = 'clWb3bSm9VcodIpKN99FQNSNIFYdSQ9IK5eHCd3k1ukM110'
	VIEW_PORT = {'width': 1430, 'height': 700}
	APP_NAME = 'ACME DEMO TEST'

# Login Form Data
	HEADERLABEL = 'Login Form'
	USERNAME = 'username'
	PASSWORD = 'password'
	ULABEL = 'Username'
	PWDLABEL = 'Password'
	LBLLOGIN = 'Log In'
	LBLCHKBOX = 'Remember Me'
	TXTUSER = 'Enter your username'
	TXTPWD = 'Enter your password'
	USERNAME_ALERT = 'Password must be present'
	PASSWORD_ALERT = 'Username must be present'
	CREDENTIALS_ALERT = "Both Username and Password must be present"


# Home Page Data
	SORTED_VALUES = ['- 320.00 USD', '- 244.00 USD', '+ 17.99 USD', '+ 340.00 USD', '+ 952.23 USD', '+ 1,250.00 USD']
	UNSORTED_VALUES = ['+ 1,250.00 USD', '+ 952.23 USD', '- 320.00 USD', '+ 17.99 USD', '- 244.00 USD', '+ 340.00 USD']
	SRCIMG1 = 'https://demo.applitools.com/img/flashSale.gif'
	SRCIMG2 = 'https://demo.applitools.com/img/flashSale2.gif'
	HOMEPAGE ='Your nearest branch closes in: 30m 5s'
